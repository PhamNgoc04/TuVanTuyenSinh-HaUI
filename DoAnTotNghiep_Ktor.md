...
LỜI CẢM ƠN 
Đầu tiên, em xin gửi lời cảm ơn đến các thầy cô trong Đại học Công nghiệp Hà Nội nói chung và các thầy cô trong khoa Công nghệ thông tin nói riêng. Cảm ơn các thầy cô đã dạy cho em rất nhiều kiến thức và kinh nghiệm trong suốt những năm đại học, giúp em có được cơ sở lý thuyết vững vàng và tạo một môi trường học tập chuyên nghiệp để em có thể phát triển bản thân và vững bước trên con đường phát triển sự nghiệp. 
...
MỤC LỤC
Chương 1: Tổng quan về hệ thống tư vấn thông tin tuyển sinh HaUI.........................................................................................7
...
Hình 2. 12 Một số thông tin từ nhà trường...................................................................................................................................33
...
Chương 1: Tổng quan về hệ thống tư vấn thông tin tuyển sinh HaUI
Tại chương 1: Tổng quan về hệ thống tư vấn thông tin tuyển sinh HaUI, em sẽ trình bày tổng quan về yêu cầu cho hệ thống tư vấn tuyển sinh đồng thời cũng khảo sát hiện trạng và so sánh với những hệ thống tư vấn hiện có. Điều này nhằm mục đích xác định những yếu điểm cũng như những ưu điểm của các hệ thống đối thủ, từ đó đề xuất những cải tiến cho hệ thống của mình. 
...
Hệ thống phục vụ cho công tác tư vấn tuyển sinh, định hướng nghề nghiệp và xem chỉ tiêu xét tuyển nhằm cung cấp những thông tin cần thiết về Đại học Công nghiệp Hà Nội ví dụ như các thông tin về chương trình học, yêu cầu đầu vào và cơ hội việc làm sau khi tốt nghiệp,....  
...
•	Đại học Công nghiệp Hà Nội (HaUI): HaUI Slink, HaUI Slink tuyển sinh - là các ứng dụng cung cấp đầy đủ các thông tin về nhà trường ví dụ như thông tin nhà trường, thông tin các ngành học, ứng dụng được Đại học Công nghiệp Hà Nội phát triển nhằm hỗ trợ thí sinh và phụ huynh trong quá trình tuyển sinh vào trường. Ứng dụng HaUI S-Link Tuyển sinh là một công cụ hữu ích giúp thí sinh và phụ huynh thuận tiện hơn trong quá trình tuyển sinh vào Đại học Công nghiệp Hà Nội. Ứng dụng cung cấp thông tin tuyển sinh đầy đủ và chính xác về các phương thức tuyển sinh, chỉ tiêu, tổ hợp xét tuyển,... của Nhà trường. đồng thời cung cấp các thông tin hỗ trợ tuyển sinh như lịch thi, hướng dẫn thí sinh,...
...
Dựa vào mô hình tổng quát của hệ thống, em lựa chọn kiến trúc hướng dịch vụ vi mô (Microservices) để phát triển hạ tầng Server và sử dụng phương pháp hướng đối tượng để phát triển ứng dụng mobile (Native Android). Ktor Backend, AI Service và Frontend sẽ giao tiếp với nhau thông qua Restful API.
1.3.2.1 Kiến trúc hệ thống phân tán (Microservices) - Khối server
Thay vì xây dựng một hệ thống máy chủ nguyên khối (Monolith), hệ thống áp dụng tư tưởng Microservices chia Backend thành 2 dịch vụ độc lập nhằm tối ưu hóa hiệu năng và vòng đời phát triển:
- Core API Service (Ktor/Kotlin): Chịu trách nhiệm trực tiếp cho thao tác CSDL MySQL, quy trình đăng nhập, xác thực và trả về dữ liệu cơ bản.
- AI Service (FastAPI/Python): Là một dịch vụ độc lập bọc mô hình học sâu (Transformer NLP và Whisper). Service này chỉ có một nhiệm vụ duy nhất là xử lý luồng âm thanh/chuỗi văn bản người dùng đặt câu hỏi và sinh ra câu trả lời tư vấn. Thiết kế này giúp tận dụng hệ sinh thái AI mạnh mẽ của Python, trong khi Ktor/Kotlin vẫn đảm đương tốt việc xử lý tải nặng nhờ Coroutines. Việc tách biệt giúp hệ thống có tính sẵn sàng cao (High Availability), nếu module AI bận rộn thì hệ thống tra cứu tra cứu tuyển sinh vẫn hoạt động mượt mà.
...
1.3.4.1 Phía server
1.3.4.1.1. Ktor (Kotlin) - Core API Server
Ktor là một framework bất đồng bộ (asynchronous) mạnh mẽ được phát triển bằng ngôn ngữ Kotlin, do chính JetBrains thiết kế. Ktor tận dụng triệt để sức mạnh của Kotlin Coroutines, mang lại khả năng xử lý hàng ngàn kết nối đồng thời với lượng CPU và RAM rất nhỏ. Ktor hoàn toàn không bị chặn luồng (non-blocking), điểm này vô cùng quan trọng đối với một hệ thống tra cứu điểm và thông tin tuyển sinh trong giờ cao điểm.
Hệ thống sử dụng Ktor để thiết lập Core API giúp ứng dụng gửi yêu cầu lấy dữ liệu như Thông tin ngành, Lịch sử trường, Quản lý tài khoản và Đăng nhập. Việc dùng Kotlin cho cả Backend (Ktor) và Frontend (Android) mang tới sự đồng nhất cao về mindset lập trình và Data class chia sẻ.

1.3.4.1.2. FastAPI (Python) - AI Microservice
Thay vì nhồi nhét code Machine Learning trực tiếp, em xây dựng riêng một AI Service bằng FastAPI (Python). FastAPI cực kỳ nhẹ, tối ưu về tốc độ theo chuẩn OpenAPI, giúp dễ dàng nhận file âm thanh (giọng nói) từ Mobile, đưa qua Whisper và Transformer xử lý, sau đó trả text câu trả lời về lại Ktor hoặc trả thẳng cho Mobile nhanh chóng.

1.3.4.1.3. MySQL - Quản trị CSDL Quan hệ
Thay vì sử dụng SQLite vốn chỉ phù hợp cho test hoặc lưu cache trên local máy khách, Data-tier của hệ thống sử dụng MySQL làm hệ quản trị cơ sở dữ liệu quan hệ cho Core Server Ktor. MySQL cung cấp tính năng khóa hàng (row-level locking), hỗ trợ giao dịch ACID an toàn tuyệt đối. Kết hợp với thư viện ORM như Ktorm hoặc Exposed trên Kotlin, hệ thống lưu trữ toàn bộ thông tin đăng nhập, thông tin khối ngành và chi phí đào tạo một cách tin cậy.
1.3.4.2 Phía Client
...
1.3.4.2.4 Retrofit + Gson (Network Layer)
Retrofit là một HTTP client type-safe cho Android và Java. Retrofit giúp phân tích các lệnh gọi API thành các function trong Kotlin Interface. Trong dự án, Retrofit được cấu hình với Gson Converter kết nối trực tiếp đến Ktor Server và FastAPI AI Server, đi kèm OkHttp Interceptor để đính kèm Token trong luồng đăng nhập tự động.

1.3.4.2.5 DataBinding / ViewBinding và Kiến trúc Clean Architecture
Hệ thống Mobile được phát triển tuân thủ nghiêm ngặt mô hình Clean Architecture (Presentation, Domain, Data Layer). Phần giao diện (Frontend) sử dụng chuẩn XML kết hợp DataBinding.
DataBinding trong Kotlin giúp liên kết trực tiếp dữ liệu từ lớp ViewModel sang tập tin XML giao diện mà không cần `findViewById` một cách dư thừa, làm giảm rủi ro NullPointerException rất hiệu quả và phân bóc tách UI ra khỏi logic (MVVM).

1.3.4.2.6 Dagger-Hilt (Dependency Injection)
Để kết nối mọi thành phần kể trên một cách mượt mà, em tích hợp Hilt - thư viện chuẩn xác nhất do Google khuyến nghị để đảm nhận Tiêm Phụ thuộc (DI). Hilt phụ trách quản lý các Singletons của Retrofit Client, khởi tạo ViewModels và cung cấp Context cho các Repositories, loại bỏ sự cồng kềnh của code module theo thiết kế cũ.
1.4 Tổng kết chương
...
Vì là hệ thống tư vấn tự động liên quan đến nhà trường Công nghệ Công nghiệp Hà Nội vậy nên em sẽ sử dụng các thông tin có sẵn tại website của nhà trường. Qua đó tổng hợp lại thành các câu hỏi và các câu trả lời tương ứng
 
Hình 2. 12 Một số thông tin từ nhà trường
...
Mô tả về hệ thống: Hệ thống được xây dựng nhằm phục vụ cho công tác tư vấn tuyển sinh, định hướng nghề nghiệp và xem chỉ tiêu xét tuyển nhằm cung cấp những thông tin cần thiết về Đại học Công nghiệp Hà Nội bao gồm các thông tin về: 
●	Thông tin về trường
...
3.1.2.0 Chức năng Đăng nhập / Đăng ký
Actor: Học sinh, sinh viên, Người quản lý
Precondition: Người dùng đã mở ứng dụng thành công
Post-condition: Hệ thống xác thực danh tính người dùng và cấp Token (Role) phù hợp
Kịch bản chính (Đăng nhập):
1. Người dùng chọn "Đăng nhập".
2. Hệ thống yêu cầu nhập Username và Password.
3. Hệ thống gửi yêu cầu lên Core API (Ktor), Ktor đối chiếu mật khẩu đã mã hóa trong MySQL.
4. Core API trả về Bearer Token và Role (Admin hoặc User).
5. App lưu Token vào bộ nhớ cục bộ (SharedPreferences/DataStore) và chuyển màn hình.
Ngoại lệ: Mật khẩu sai -> Thông báo lỗi.

Kịch bản chính (Đăng ký):
1. Người dùng chọn "Tạo tài khoản".
2. Hệ thống (Ktor) kiểm tra Username đã tồn tại trong MySQL hay chưa. Nếu chưa, tiến hành mã hóa Password bằng BCrypt.
3. Ktor lưu user mới vào DB và thông báo thành công.
...
Post-condition	Học sinh, sinh viên đã xem thông tin về trường HaUI
Kịch bản chính	1.	Học sinh, sinh viên chọn chức năng "Xem thông tin trường"
2.	Hệ thống hiển thị thông tin về trường HaUI trên giao diện ứng dụng, bao gồm:
-	Lịch sử và thông tin chung về trường.
-	Thông tin liên hệ và địa chỉ trường. -	Các thông tin liên quan khác.
3.	Học sinh, sinh viên đọc và xem thông tin về trường HaUI
...
2.	Hệ thống hiển thị danh sách các ngành học trong trường HaUI với các thông tin sau:
-	Tên ngành học.
...
tenNghe	varchar(255)	tên ngành nghề
mucLuong	varchar(255)	mức lương trung bình
tinhTrang	varchar(255)	tình trạng công việc hiện tại
NganhHocmaNganh	varchar(10)	khoá ngoại liên kết với bảng NganhHoc
4.4 Bảng ChiTieu
Tên trường	Thuộc tính	Ghi chú
...
4.10 Bảng KhoaHoc (Bảng trung gian N-N)
Tên trường	Thuộc tính	Ghi chú
...
password_hash	varchar(255)	mật khẩu (cần hash BCrypt/SHA256)
role	varchar(20)	vai trò phân quyền (ADMIN / USER)
...
+ Xây dựng Core API Backend bảo mật với Ktor (Kotlin) và CSDL MySQL
+ Tách biệt phân tán Microservice AI với FastAPI (thay vì dồn cục)
+ Thư viện Android Jetpack
+ Áp dụng kiến trúc Clean Architecture kết hợp MVVM và Dagger-Hilt ● Cải thiện kỹ năng viết và trình bày tài liệu
B. Hạn chế
...
Vì là hệ thống được triển khai trên ứng dụng di động Android, vậy nên việc triển khai hệ thống này sẽ cần có Android Studio. Có thể tải trực tiếp trên trang chủ.
	...
	1.2.1. Cài đặt Core API Backend (Ktor) & MySQL
- Mở Server Backend bằng IntelliJ IDEA (yêu cầu cài đặt Java JDK 17).
- Cấu hình file `application.conf` để trỏ tới chuỗi kết nối MySQL cục bộ.
- Khởi chạy Database MySQL bằng lệnh `mysql -u root -p` và tạo CSDL `CREATE DATABASE tuyensinh_dack;`.
- Chạy Ktor Server bằng Gradle: `./gradlew run`. Server mặc định phát sóng ở `127.0.0.1:8080`.

1.2.1b. Cài đặt AI Service (FastAPI)
Để cài đặt FastAPI phục vụ xây dựng Dịch vụ AI phân tán, em tạo một không gian làm việc với Python.
Tạo một thư mục dành cho FastAPI: `mkdir fast-ai-service && cd fast-ai-service`.
Tiếp theo thiết lập môi trường ảo và kích hoạt: `python3 -m venv venv` và `source venv/bin/activate`.
Sau đó cài đặt FastAPI và uvicorn: `pip3 install fastapi uvicorn`.
Để khởi chạy AI Server (cổng 8000), chạy lệnh `uvicorn main:app --reload`. Khi đó màn hình hiển thị:
...
d:\Mobile_Kotlin_DevPro\DACK\
│
├── 📱 HaUI_TuyenSinh_Android/   (Chứa Frontend: Android App viết bằng Kotlin + XML)
│
├── ⚙️ HaUI_Core_Ktor/           (Chứa Backend: API Server với Ktor + kết nối MySQL)
│
├── 🧠 HaUI_AI_Service/          (Chứa AI: Python FastAPI để xử lý Whisper & Transformer)
│
├── DoAnTotNghiep_Ktor.md        (Báo cáo bản chuẩn thiết kế)
└── DoAnTotNghiep.md             (Báo cáo bản gốc cục bộ)
